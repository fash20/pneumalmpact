import { Avatar, Button, Divider, IconButton, useMediaQuery } from "@material-ui/core";
...
const Explore = () => {
  return (
    <div className=" grid grid-cols-1 px-5 gap-y-10 md:gap-y-20 pt:8 lg:px-6">
      ...
      <div className="mt-10">
        ...
        <div className="grid justify-center items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7">
          {
            course.map((item, key)=>(
              <Course title={item.title} image={item.image} key={key} properties={item.properties}  />
            ))
          }
        </div>
      </div>
      <div className="flex flex-col justify-center items-center space-y-6">
        ...
        <div className="grid justify-center items-center grid-cols-1 md:grid-cols-2 md:items-start md:justify-left lg:grid-cols-3 gap-5 ">
          <CourseCardBrief />
          <CourseCardBrief />
          <CourseCardBrief />
          <CourseCardBrief />
...
const CourseCard = ({ title, src }: CourseCardProps) => {
  return (
    <div
     className="flex flex-col space-y-3 w-[80px] mb:w-[100px]  sm:w-[150px] lg:w-[190px] shrink-0 "
     >
      <img
...
const Course = ({title, subtitle,image, detail, properties}:ICourseProps) => {
  return (
    <div className="w-[287px] md:w-[250px] h-[250px]  p-3 grid  items-center gap-2 border-[1px] border-grayMarginColor">
...
export const Tag = ({ title, className }: TagIconProp) => {
  ...
  className === undefined
    ...
    : (_className =
      `text-[11px] text-white font-interlight w-fit  px-2 py-1 bg-[${rand}]` +
        className);
  return <span className={_className} style={{backgroundColor:tagColors[rand]}}>{title}</span>;
};
  ...
  return (
    <div className="flex justify-center items-center space-x-4 p-5 rounded-lg border-[1px] border-grayMarginColor w-fit bg-white shadow-sm">
      <div className="w-[60px] h-[60px] rounded-[50%] bg-[#f2f2f2] flex justify-center items-center">
        <Avatar sizes={isScreenSmall? 'medium':'large'} src={mountainImg} alt='avatar'/>
      </div>

      <div className="flex flex-col space-3">
        <span className="text-lg md:text-2xl lg:text-2xl">Email Personalisation</span>
